# Contributing to ep_desktop_notifications

Thanks for helping improve `ep_desktop_notifications` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_desktop_notifications` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_desktop_notifications from your local clone
pnpm run plugins i --path ../ep_desktop_notifications

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_desktop_notifications/` are picked up after a server restart.
...
* Etherpad version, ep_desktop_notifications version, browser + OS
* Steps to reproduce
